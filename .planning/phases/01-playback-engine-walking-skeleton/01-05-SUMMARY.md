...
provides:
  - "phase1Stations: the four owner-verified release stations (curated:bnr-horizont, curated:radio1, curated:bg-radio, curated:energy) with 11 official streams in priority order, each commented with its source and verifiedAt 2026-09-25"
  ...
  - "test/features/catalog/phase1_stations_test.dart (20 tests) and test/features/catalog/station_test.dart (10 tests)"
...
key-decisions:
  - "Phase 1 ships 4 stations, not 6. N-JOY is excluded because neither official bTV Radio endpoint played in VLC. Радио Витоша is excluded because no official stream was found (option-c). No unofficial substitute is used for either (D-03)."
...
coverage:
  - id: D1
    description: "Release list is exactly the four owner-verified stations (flagship first), with curated unique ids, at least one stream each, the owner's exact URLs in priority order, Cyrillic and Latin names, and БГ Радио's tracer-verified MP3 as primary"
    requirement: PLAY-08
    verification:
      - kind: unit
  ...
  - id: D2
    description: "Reduced D-03 coverage matrix: MP3, AAC, HLS and plain http:// in release; no .pls/.m3u (N-JOY excluded) and no cp1251 (no probe), each asserted with the owner decision and date; no excluded station or rejected host ships"
    requirement: STRM-01
    verification:
      - kind: unit
  ...
  - id: D5
    ...
    human_judgment: false
  - id: D6
    description: "Full suite green with the new list, including the 8-step tracer (БГ Радио found by text among 7 debug-build stations)"
    verification:
      - kind: integration
        ref: "flutter test (191 tests, All tests passed!) incl. test/app/tracer_e2e_test.dart"
        status: pass
      - kind: other
    ...
    human_judgment: false
  - id: D7
    description: "On the owner's phone: every release station plays within about 5 s (Хоризонт via HLS, at least one http:// station); in a debug build the ТЕСТ stations are listed and the HLS-sniff entry plays as HLS; a release build lists no ТЕСТ station"
    requirement: STRM-02
    verification: []
    human_judgment: true
...
**The station list now holds four owner-verified stations: БНР Хоризонт (2 HLS streams), then Радио 1, БГ Радио and Радио Енерджи (3 streams each). All 11 are official streams in priority order, covering MP3, AAC, HLS and plain http://. Debug and profile builds add dead-primary, slow-primary and Хоризонт HLS-sniff test stations, and a `kReleaseMode`-gated directory keeps them out of release builds.**

## Performance

- **Duration:** 6 min for this continuation (Task 2). Task 1 was the owner's checkpoint on 2026-09-25.
- **Started:** 2026-09-25T16:03:51Z
- **Completed:** 2026-09-25T16:10:06Z
- **Tasks:** 2 of 2: Task 1 was the owner's decision, resolved; Task 2 was TDD (RED, then GREEN; no refactor needed)
- **Files modified:** 7 (3 created, 4 modified)

## Owner's D-02 Table (Task 1 resolution)

The owner played every URL in VLC on 2026-09-25 and captured the БНР URLs from bnr.bg's own player.
...
| N-JOY `curated:njoy` | — | http://live.btvradio.bg/njoy.mp3.m3u and http://live.btvradio.bg/njoy.mp3 | — | — | research | 2026-09-25 | EXCLUDED: neither plays |
| Радио Витоша `curated:vitosha` | — | none found | — | — | — | 2026-09-25 | EXCLUDED: option-c |

Codec and bitrate follow the endpoint names in the research matrix. The owner did not report an ffprobe run, so HE-AAC versus LC is still unconfirmed for the StreamTheWorld `.aac` streams.
...
**Consequence:** with N-JOY excluded, no release station uses a `.pls`/`.m3u` wrapper. The wrapper format (D-03, STRM-03) is covered by the 01-04 resolver tests only. A candidate for later owner verification is `http://play.global.audio/bgradio128.m3u`. It was **not** added because it is unverified.

## Accomplishments

- **Release list (D-01, D-04):** four stations, in display order БНР Хоризонт, Радио 1, БГ Радио, Радио Енерджи. Each carries the owner's verified streams in the owner's order, with the HTTPS StreamTheWorld AAC first where one exists. Each stream literal has a `// Source: …; verifiedAt 2026-09-25 (VLC)` comment. The file header records the exclusions and rejections, without naming the rejected hosts.
- **Debug stations (D-05):**
...
- **Tests:** 30 new tests (20 in `phase1_stations_test.dart`, 10 in `station_test.dart`). The full suite of 191 tests passes, including all 8 tracer steps and `_lastStation` resumption. `flutter analyze` and `dart analyze` report no issues. No test touches the network.
...
2. **Task 2: the owner-verified stations and debug-only test stations with ordered fallback streams.** RED `ad69fee` (test), GREEN `2dd6a7c` (feat)

**Plan metadata:** see the `docs(01-05): complete …` commit.

## TDD Gate Compliance

| Task | RED | GREEN | REFACTOR | Evidence |
|------|-----|-------|----------|----------|
| 2 | `ad69fee`: 15 of 28 failed, all on assertions. | `2dd6a7c` | not needed | `check tdd-red-evidence` returned RED_EVIDENCE_OK for 5 targets: the four-station list, the empty-streams ArgumentError, the debug ids, import isolation, and the D-03 matrix |

The 13 tests that passed in RED fall into three groups:
...
As in 01-03, 01-04 and 01-06, the RED run used `flutter test --reporter json`, converted into TAP for the checker. The RED commit's only scaffolding was the ignored `includeDebug` parameter.

## Files Created/Modified

- `lib/features/catalog/data/phase1_stations.dart`: the four owner-verified stations, with 11 streams and their source/verifiedAt comments.
...
See `key-decisions` in the frontmatter. In short: 4 stations by owner decision; Хоризонт is HLS-only; no wrapper and no cp1251 in the release data, both covered by tests; and the debug list is const-gated on `kReleaseMode`.

## Deviations from Plan

### Owner-decision deviations (recorded, not failures)

**1. Four stations instead of six**
- **Why:** The owner excluded N-JOY (no endpoint plays) and Радио Витоша (option-c) on 2026-09-25.
- **What changed:**
  - The tests assert the four ids, the owner's exact URL lists, and the reduced D-03 matrix. Each reduced-matrix test names the decision and date.
  - The plan's "N-JOY via its .m3u" human check does not apply.
  - The must-have truth about "a .pls or .m3u wrapper" in the release list is met by the 01-04 resolver tests only.
  - The artifact description "six-station release list" now reads four.
- **Files:** phase1_stations.dart, phase1_stations_test.dart, station_test.dart
- **Committed in:** ad69fee, 2dd6a7c

**2. Хоризонт ships HLS only**
- **Why:** The planned stream order was HLS, then 8011 AAC, then 8011 MP3. The owner rejected both 8011 mounts and verified a second HLS endpoint (e106-ts.cdn.bg) from bnr.bg's player instead.
...
- **Issue:** Four home-screen and mini-player tests used `StationDirectory.phase1().all.first` and expected one ListTile, a `ListPlayContext` of `[_station.id]`, and "БГ Радио" labels. The flagship Хоризонт is now first, and debug builds list 7 stations.
...
- **Verification:** the full suite passes (191 tests).
- **Committed in:** 2dd6a7c
...
**Impact on plan:** The owner decisions reduce the release lineup and remove the on-device wrapper case. That coverage is now test-only, as recorded above. The auto-fixes tighten the plan's own invariants (T-05-02 and the 01-01 empty-streams rule). There is no scope creep.
...
- T-05-01 is mitigated by the owner's VLC verification and the per-stream source/verifiedAt comments.
- T-05-02 is mitigated by the const gate, the unit tests and the grep gates, with the 01-02 CI `strings` check as a backstop.
- T-05-03 (http:// media) is accepted: https StreamTheWorld streams are listed first where they exist.
...
- 01-09 (reconnect and rotation) has real ordered fallbacks (2 or 3 per station) and the dead-primary and slow-primary debug stations to exercise rotation on a phone.
...
- End-of-phase UAT (D7, and 01-04's D7):
  - In a release build, play all four stations for about 30 s each, including Хоризонт via HLS and at least one http:// stream.
...
- Open for the owner, not blocking:
  ...
  - Find an official Радио Витоша stream, and a working N-JOY endpoint, in Phase 2 curation.
