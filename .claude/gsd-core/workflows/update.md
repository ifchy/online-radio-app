...
for cand in \
  ...
  "/Users/alabala/Documents/projects/personal/online-radio-app/.claude/gsd-core/bin/gsd-tools.cjs"; do
...
The `GSD_DIR` value emitted by `get_installed_version` (line 4) resolves to the runtime-specific config dir (`/Users/alabala/Documents/projects/personal/online-radio-app/.claude/`, `~/.gemini/`, `~/.codex/`, etc.), so the script invocation works for every runtime — not just Claude. An unresolved target exits in `get_installed_version` before this step.
...
global: `/Users/alabala/Documents/projects/personal/online-radio-app/.claude/`, `~/.config/opencode/`, `~/.opencode/`, `~/.gemini/`, `~/.config/kilo/`, or `~/.codex/`
