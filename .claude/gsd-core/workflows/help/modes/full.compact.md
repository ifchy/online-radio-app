...
**`/gsd-capture --note <text>`** — Zero-friction timestamped note to `.planning/notes/` (or `/Users/alabala/Documents/projects/personal/online-radio-app/.claude/notes/` globally). Subcommands: append (default), list, promote (note → todo). Works without a project.
